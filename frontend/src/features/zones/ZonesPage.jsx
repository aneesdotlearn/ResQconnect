...
function ZoneModal({ zone, onClose, onSave }) {
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-3 py-4"
      onClick={onClose}
    >
      <div
        className="glass-card p-0 relative bg-white/90 w-full max-w-md flex flex-col animate-slide-up"
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-neutral-100 flex-shrink-0">
          ...
          <button
            onClick={onClose}
            className="p-1.5 hover:bg-neutral-100 rounded-lg text-charcoal/50 transition-colors"
          >
            <X size={18} />
          </button>
        </div>
        ...
        <div className="overflow-y-auto flex-1 px-5 py-4">
          <form onSubmit={handleSubmit} className="space-y-4" id="zone-form">
            ...
            <div>
              <label className="block text-sm font-medium text-charcoal/70 mb-1">
                Radius (meters) <span className="text-error-500">*</span>
              </label>
              <input
                type="number"
                value={form.radius}
            ...
            <div>
              <label className="block text-sm font-medium text-charcoal/70 mb-2">
                Zone Location <span className="text-error-500">*</span>
              </label>
              ...
              <div className="flex gap-2 mb-2">
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-1.5 py-2 text-sm bg-accent-50 text-accent-600 hover:bg-accent-100 rounded-xl font-medium transition-colors"
                >
              ...
              {form.coordinates ? (
                <div className="flex items-center gap-2 text-xs text-success-500 bg-green-50 border border-success-500/20 px-3 py-2 rounded-xl mb-2">
                  <span className="w-2 h-2 bg-success-500 rounded-full flex-shrink-0" />
                  <span>
                  ...
                  <button
                    ...
                    className="ml-auto text-success-500 hover:text-error-500 transition-colors"
                  >
                    <X size={12} />
                  </button>
                </div>
              ) : (
                <div className="flex items-center gap-2 text-xs text-warn-500 bg-orange-50 border border-warn-500/20 px-3 py-2 rounded-xl mb-2">
                  <span className="w-2 h-2 bg-warn-500 rounded-full flex-shrink-0" />
            ...
            <div className="space-y-2">
              {[
                ...
                <label
                  key={key}
                  className="flex items-center justify-between p-3 bg-white/50 hover:bg-white/70 rounded-xl cursor-pointer transition-colors"
                >
                  <div>
                    <p className="text-sm font-medium text-charcoal/70">{label}</p>
                    <p className="text-xs text-charcoal/40">{sub}</p>
                  </div>
                  <input
                    type="checkbox"
                    checked={form[key]}
                  ...
                  />
                  {form[key]
                    ? <ToggleRight size={24} className="text-accent-500 flex-shrink-0 ml-3" />
        ...
        <div className="flex gap-3 px-5 py-4 border-t border-neutral-100 flex-shrink-0 bg-white/60 rounded-b-2xl">
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="font-display font-bold text-2xl text-charcoal">Safe Zones</h1>
      ...
      {loading ? (
        ...
        <div className="glass-card text-center py-12">
          ...
          <p className="font-semibold text-charcoal/70 mb-1">No safe zones yet</p>
        ...
        <div className="space-y-3">
          {zones.map((zone) => (
            <div
              key={zone._id}
              className={`glass-card flex items-center gap-4 transition-opacity ${!zone.isActive ? 'opacity-50' : ''}`}
            >
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${zone.isActive ? 'bg-green-100' : 'bg-neutral-100'}`}>
                <Shield size={20} className={zone.isActive ? 'text-success-500' : 'text-neutral-400'} />
              </div>
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-charcoal truncate">{zone.name}</p>
              ...
              <div className="flex items-center gap-1 flex-shrink-0">
                ...
                <button
                  ...
                  title="Edit"
                >
                  <Shield size={16} className="text-charcoal/50" />
                </button>
                <button
                  ...
                  title="Delete"
                >
                  <Trash2 size={16} className="text-error-500/70" />
                </button>
              </div>
            </div>
          ))}
