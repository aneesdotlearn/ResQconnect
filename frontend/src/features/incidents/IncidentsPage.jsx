...
import { FileText, Plus, X, Save, MapPin } from 'lucide-react';
import { formatDistanceToNow } from 'date-fns';

const TYPES = ['harassment', 'stalking', 'assault', 'theft', 'threat', 'suspicious_activity', 'other'];
const SEVERITIES = ['low', 'medium', 'high', 'critical'];

const STATUS_BADGE   = { open: 'badge-error', under_review: 'badge-warn', resolved: 'badge-success', closed: 'badge-gray' };
const SEVERITY_BADGE = { low: 'badge-accent', medium: 'badge-warn', high: 'badge-error', critical: 'badge-error' };
...
function NewIncidentModal({ onClose, onSave }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 px-4 overflow-y-auto py-4" onClick={onClose}>
      <div className="glass-card bg-white/90 w-full max-w-lg p-6 animate-slide-up" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-5">
          <h2 className="font-display font-bold text-lg text-charcoal">Report Incident</h2>
          <button onClick={onClose} className="p-1.5 hover:bg-neutral-100 rounded-lg text-charcoal/50"><X size={18} /></button>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-charcoal/70 mb-1">Title</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-sm font-medium text-charcoal/70 mb-1">Severity</label>
          ...
          <div className="flex items-center gap-3">
            <button type="button" onClick={getLocation} disabled={locating}
              className="flex items-center gap-2 px-3 py-2 bg-neutral-100 hover:bg-neutral-200 rounded-xl text-sm font-medium text-charcoal/70 transition-colors">
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="font-display font-bold text-2xl text-charcoal">Incident Reports</h1>
          <p className="text-charcoal/60 text-sm mt-0.5">{total} incident{total !== 1 ? 's' : ''} reported</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="glass-card text-center py-16">
          ...
          <p className="font-semibold text-charcoal/70 mb-1">No incidents found</p>
          <p className="text-sm text-charcoal/40">Report an incident to keep a record of safety concerns.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {incidents.map((inc) => (
            <div key={inc._id} className="glass-card">
              <div className="flex flex-wrap items-start justify-between gap-2 mb-2">
                <h3 className="font-semibold text-charcoal">{inc.title}</h3>
              ...
              <p className="text-sm text-charcoal/70 line-clamp-2">{inc.description}</p>
              <div className="flex flex-wrap gap-3 mt-3 text-xs text-charcoal/40">
          ...
          {total > 10 && (
            <div className="flex justify-center gap-2 pt-2">
              ...
              <span className="text-sm text-charcoal/50 self-center">Page {page} of {Math.ceil(total / 10)}</span>
