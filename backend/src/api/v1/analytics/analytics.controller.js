...
exports.getUserAnalytics = async (req, res) => {
  ...
  const [sosSummary, incidentSummary, sosMonthly, riskStats] = await Promise.all([
    ...
    SOS.aggregate([
      { $match: { user: userObjId, createdAt: { $gte: new Date(Date.now() - 365 * 24 * 60 * 60 * 1000) } } },
  ...
  const data = {
    ...
    incidentSummary,
    sosMonthly,
    riskStats: riskStats[0] || { avgRiskScore: 0, maxRiskScore: 0, minRiskScore: 0, totalWithRisk: 0 },
  };
